...
jQuery.fn.colourPicker = function (conf) {
	...
	return this.each(function () {
		...
		var inputTemp	= jQuery('<input type="text" name="' + select.attr('name') + '_temp" value="" style="cursor:pointer;" />').insertAfter(select);
		var input	= jQuery('<input type="hidden" name="' + select.attr('name') + '" value="' + select.val() + '" />').insertAfter(select);
		
		//prevent text entry in input temp field
		inputTemp.keydown(
    function(event)
    {
			return false;
		...
		if (config.inputBG) {
			input.change(function () {
				...
				inputTemp.css({background: '' + input.val(), color: '#' + hexInvert(input.val().replace('#',''))});
			});
		...
		//icon.click(function () {
		//changing icon click to inputTemp click
